  ...
  return <Phone label={isNew ? "Entry Detail (Text)" : fromRecording ? "Entry Detail (From Recording)" : "Entry Detail"}>
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", animation: "slideUp 0.3s ease both" }}>
      ...
      <div style={{ flex: 1, overflow: "auto", padding: "8px 20px 20px" }}>
        ...
        <div style={{ background: C.card, backgroundImage: paperTex, borderRadius: 14, padding: "16px 18px", border: `1px solid ${C.border}`, marginBottom: 12 }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: C.text, marginBottom: 4 }}>{e.date}</div>
          <div style={{ fontSize: 13, color: C.textMuted, marginBottom: 10 }}>{e.time}</div>
          <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>{e.children.map(c => { const ch = CHILDREN.find(x => x.name === c); const age = e.ages?.[c]; return <span key={c} style={{ display: "inline-flex", alignItems: "center", gap: 4 }}><CPill name={c} color={ch?.color || C.general} />{age && <span style={{ fontSize: 11, color: C.textMuted }}>{age}</span>}</span>; })}</div>
        </div>
        ...
        <div style={{ background: C.card, backgroundImage: paperTex, borderRadius: 14, padding: "18px", border: `1px solid ${C.border}`, minHeight: 180, fontSize: 15, color: C.text, lineHeight: 1.65, fontFamily: "'Georgia', serif" }}>{isNew ? <span style={{ color: C.textMuted, fontStyle: "italic" }}>Start typing your memory...</span> : e.text}</div>
        {!isNew && <div style={{ textAlign: "center", marginTop: 10 }}><span style={{ fontSize: 11, color: C.textMuted, fontWeight: 500 }}>All changes saved</span></div>}
      </div>
      {e.hasAudio && !isNew && <div style={{ padding: "12px 20px 24px", display: "flex", alignItems: "center", gap: 12, borderTop: `1px solid ${C.border}`, background: C.bg }}>
        <div style={{ width: 36, height: 36, borderRadius: 18, background: C.accentSoft, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}><I.Play /></div>
...
function CoreMem({ onNav }) {
  ...
  return <Phone label="♡ Core Memories">
    <style>{gs}</style>
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <div style={{ padding: "6px 20px", display: "flex", alignItems: "center" }}><div style={{ cursor: "pointer" }} onClick={() => onNav("home")}><I.Back /></div><span style={{ flex: 1, textAlign: "center", fontSize: 16, fontWeight: 700, color: C.text }}>♡ Core Memories</span><div style={{ width: 22 }} /></div>
      <div style={{ padding: "10px 20px 6px", display: "flex", gap: 8, overflowX: "auto" }}>{CHILDREN.map(c => <button key={c.name} onClick={() => setTab(c.name)} style={{ padding: "7px 16px", borderRadius: 20, cursor: "pointer", fontSize: 13, fontWeight: 700, whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: 5, border: tab === c.name ? `2px solid ${c.color}` : "2px solid transparent", background: tab === c.name ? c.color + "20" : C.card, color: tab === c.name ? c.color : C.textMuted, boxShadow: tab === c.name ? `0 2px 8px ${c.color}18` : "0 1px 3px rgba(44,36,32,0.04)" }}>{c.name !== "All" && <Dot color={c.color} sz={8} />}{c.name}</button>)}</div>
      <div style={{ flex: 1, overflow: "auto", padding: "6px 16px 20px" }}>{f.length > 0 ? f.map((e, i) => <ECard key={e.id} entry={e} onClick={() => onNav("detail", e)} delay={i * 60} />) : <div style={{ textAlign: "center", padding: "60px 20px" }}><div style={{ marginBottom: 12 }}><I.Heart filled={false} sz={32} /></div><div style={{ color: C.textMuted, fontSize: 14, lineHeight: 1.6 }}>Tap the heart on any entry to save it as a Core Memory.</div></div>}</div>
    </div>
  </Phone>;
}
...
function Settings({ onNav }) {
  const secs = [
    { t: "Children", i: ["Emma — Born Oct 15, 2023", "Liam — Born Jan 3, 2022", "Nora — Born Jun 20, 2025", "+ Add Child"] },
    { t: "Reminder", i: ["Time: 8:30 PM", "Enabled: On"] },
    { t: "Subscription", i: ["Plan: Core Memories Premium", "Manage Subscription"] },
    { t: "Data & Privacy", i: ["Export All Entries", "Recently Deleted", "Delete Account"] },
    { t: "About", i: ["Version 1.0.0", "Privacy Policy", "Terms of Service", "Contact Support"] },
  ];
  return <Phone label="Settings">
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <div style={{ padding: "6px 20px", display: "flex", alignItems: "center" }}><div style={{ cursor: "pointer" }} onClick={() => onNav("home")}><I.Back /></div><span style={{ flex: 1, textAlign: "center", fontSize: 16, fontWeight: 700, color: C.text }}>Settings</span><div style={{ width: 22 }} /></div>
      <div style={{ flex: 1, overflow: "auto", padding: "8px 20px 20px" }}>{secs.map(s => <div key={s.t} style={{ marginBottom: 20 }}><span style={{ fontSize: 12, fontWeight: 700, color: C.textMuted, letterSpacing: 0.8, textTransform: "uppercase" }}>{s.t}</span><div style={{ background: C.card, borderRadius: 14, border: `1px solid ${C.border}`, marginTop: 8, overflow: "hidden" }}>{s.i.map((item, i) => <div key={i} style={{ padding: "13px 16px", fontSize: 14, color: item.startsWith("+") || item === "Recently Deleted" ? C.accent : C.text, fontWeight: item.startsWith("+") ? 600 : 400, borderBottom: i < s.i.length - 1 ? `1px solid ${C.border}` : "none", cursor: "pointer" }}>{item}</div>)}</div></div>)}</div>
    </div>
  </Phone>;
}
...
function Notif({ onNav }) {
  return <Phone label="Push Notification">
    <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ width: "100%", background: "rgba(255,255,255,0.92)", backdropFilter: "blur(20px)", borderRadius: 20, padding: "16px 18px", boxShadow: "0 4px 24px rgba(44,36,32,0.12)", border: `1px solid ${C.border}` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}><div style={{ width: 32, height: 32, borderRadius: 8, background: C.accent, display: "flex", alignItems: "center", justifyContent: "center" }}><I.Mic sz={16} /></div><div><div style={{ fontSize: 13, fontWeight: 700, color: C.text }}>Core Memories</div><div style={{ fontSize: 11, color: C.textMuted }}>now</div></div></div>
        <p style={{ fontSize: 14, color: C.text, margin: "0 0 14px", lineHeight: 1.4 }}>What did your little one do today that made you smile?</p>
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => onNav("recording")} style={{ flex: 1, padding: "9px 0", borderRadius: 10, background: C.accent, color: "white", border: "none", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Record</button>
          <button style={{ flex: 1, padding: "9px 0", borderRadius: 10, background: C.tag, color: C.textSoft, border: "none", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Remind me later</button>
        </div>
...
const CONNECTIONS = {
  onboarding: [
    { from: "ob_signin", to: "ob_child", label: "continue", type: "forward" },
    { from: "ob_child", to: "ob_mic", label: "continue", type: "forward" },
    { from: "ob_mic", to: "ob_notif", label: "allow", type: "forward" },
    { from: "ob_notif", to: "ob_record", label: "continue", type: "forward" },
    { from: "ob_record", to: "ob_saved", label: "voice", type: "forward" },
    { from: "ob_record", to: "ob_write", label: "write instead", type: "forward" },
    { from: "ob_write", to: "ob_saved", label: "save", type: "forward" },
    { from: "ob_saved", to: "ob_paywall", label: "continue", type: "forward" },
  ],
  main: [
    { from: "home", to: "recording", label: "tap mic", type: "forward" },
    { from: "home", to: "search", label: "tap search", type: "forward" },
    { from: "home", to: "corememories", label: "tap ♡", type: "forward" },
    { from: "home", to: "settings", label: "tap gear", type: "forward" },
    { from: "home", to: "detail", label: "tap card", type: "forward" },
    { from: "recording", to: "detail", label: "save entry", type: "forward" },
    { from: "search", to: "detail", label: "tap result", type: "forward" },
    { from: "corememories", to: "detail", label: "tap card", type: "forward" },
    { from: "notification", to: "recording", label: "tap record", type: "accent" },
    { from: "notification", to: "home", label: "open app", type: "forward" },
    { from: "detail", to: "home", label: "back", type: "return" },
    { from: "recording", to: "home", label: "cancel", type: "return" },
    { from: "search", to: "home", label: "back", type: "return" },
    { from: "corememories", to: "home", label: "back", type: "return" },
...
function FlowMap({ mode, onSelectScreen, selectedScreen, onClose }) {
  ...
  const Defs = () => (
    <defs>
      <marker id="af" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto"><path d="M1,0.8 L7,3 L1,5.2" fill="none" stroke={C.textSoft} strokeWidth="1.3" strokeLinecap="round" /></marker>
      <marker id="aa" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto"><path d="M1,0.8 L7,3 L1,5.2" fill="none" stroke={C.accent} strokeWidth="1.3" strokeLinecap="round" /></marker>
      <marker id="ar" markerWidth="7" markerHeight="5" refX="6" refY="2.5" orient="auto"><path d="M1,0.5 L6,2.5 L1,4.5" fill="none" stroke={C.textMuted} strokeWidth="1" strokeLinecap="round" opacity="0.5" /></marker>
      <filter id="ns" x="-4%" y="-8%" width="108%" height="120%"><feDropShadow dx="0" dy="1" stdDeviation="2" floodColor="rgba(44,36,32,0.06)" /></filter>
    </defs>
  );

  const NCard = ({ id, x, y, w, h, isHub, isExt, isCore }) => {
    const sel = selectedScreen === id;
    return (
      <g onClick={() => onSelectScreen(selectedScreen === id ? null : id)} style={{ cursor: "pointer" }}>
        <rect x={x} y={y} width={w} height={h} rx={isExt ? 14 : 10}
          fill={sel ? C.accentSoft : C.card}
          stroke={sel ? C.accent : isHub ? C.text : isExt ? C.textMuted : isCore ? C.textSoft : C.border}
          strokeWidth={sel ? 2.5 : isHub || isCore ? 2 : 1.5}
          strokeDasharray={isExt ? "6 3" : "none"} filter="url(#ns)" />
        <text x={x + 12} y={y + h / 2 + 1} fontSize={12} dominantBaseline="central">{ico(id)}</text>
        <text x={x + 28} y={y + h / 2 + 1} fontSize={11} fontWeight={700} fill={sel ? C.accent : C.text}
          dominantBaseline="central" fontFamily={F}>{lbl(id)}</text>
      </g>
    );
  };

  // Connection path: computes exit/entry points and draws a cubic bezier
  const Conn = ({ from, to, type, label, positions }) => {
    const pf = positions[from], pt = positions[to];
    if (!pf || !pt) return null;
    const isRet = type === "return";
    const isAcc = type === "accent";
    ...
    const fcx = pf.x + pf.w / 2, fcy = pf.y + pf.h / 2;
    const tcx = pt.x + pt.w / 2, tcy = pt.y + pt.h / 2;
    const retOff = isRet ? 6 : 0;

    let sx, sy, tx, ty, side;
    if (Math.abs(tcx - fcx) > Math.abs(tcy - fcy) * 0.6) {
      if (tcx > fcx) { sx = pf.x + pf.w; sy = fcy + retOff; tx = pt.x; ty = tcy + retOff; side = "h"; }
      else { sx = pf.x; sy = fcy + retOff; tx = pt.x + pt.w; ty = tcy + retOff; side = "h"; }
    } else {
      if (tcy > fcy) { sx = fcx + retOff; sy = pf.y + pf.h; tx = tcx + retOff; ty = pt.y; side = "v"; }
      else { sx = fcx + retOff; sy = pf.y; tx = tcx + retOff; ty = pt.y + pt.h; side = "v"; }
    }
    ...
    const color = isAcc ? C.accent : isRet ? C.textMuted : C.textSoft;
    const mid = side === "h"
      ? { x: sx + dx * 0.5, y: (sy + ty) / 2 }
      : { x: (sx + tx) / 2, y: sy + dy * 0.5 };

    return (
      <g>
        <path d={d} fill="none" stroke={color}
          strokeWidth={isRet ? 1 : 1.5}
          strokeDasharray={isRet ? "5 3" : "none"}
          strokeOpacity={isRet ? 0.35 : isAcc ? 0.8 : 0.6}
          markerEnd={isRet ? "url(#ar)" : isAcc ? "url(#aa)" : "url(#af)"} />
        {label && !isRet && (
          <g>
            <rect x={mid.x - label.length * 2.6 - 5} y={mid.y - 7} width={label.length * 5.2 + 10} height={14} rx={4} fill={C.bg} fillOpacity={0.92} />
            <text x={mid.x} y={mid.y + 1} textAnchor="middle" dominantBaseline="central"
              fontSize={8} fontWeight={600} fill={isAcc ? C.accent : C.textMuted} fontFamily={F}>{label}</text>
          </g>
        )}
      </g>
    );
  ...
  // Group outline with label
  const GBox = ({ x, y, w, h, label, strong }) => (
    <g>
      <rect x={x} y={y} width={w} height={h} rx={14}
        fill="none" stroke={strong ? C.textSoft : C.border}
        strokeWidth={strong ? 1.5 : 1} strokeDasharray={strong ? "none" : "6 3"} strokeOpacity={strong ? 0.5 : 0.6} />
      <text x={x + 10} y={y + 13} fontSize={8} fontWeight={700} fill={C.textMuted} fontFamily={F} letterSpacing="0.6">{label.toUpperCase()}</text>
    </g>
  );
  ...
  const MainMap = () => {
    const P = {
      home:         { x: 50,  y: 120, w: 130, h: 46 },
      notification: { x: 50,  y: 310, w: 130, h: 46 },
      recording:    { x: 310, y: 70,  w: 130, h: 46 },
      detail:       { x: 310, y: 220, w: 130, h: 46 },
      search:       { x: 560, y: 50,  w: 150, h: 46 },
      corememories: { x: 560, y: 170, w: 150, h: 46 },
      settings:     { x: 560, y: 310, w: 130, h: 46 },
    };
    const W = 760, H = 400;
    ...
    return (
      <svg viewBox={`0 0 ${W} ${H}`} style={{ display: "block", width: "100%", maxWidth: W, height: "auto" }}>
        <Defs />
        {/* Group outlines */}
        <GBox x={30} y={22} w={170} h={186} label="Entry Points" />
        <GBox x={30} y={280} w={170} h={96} label="External Trigger" />
        <GBox x={290} y={22} w={170} h={270} label="Core Capture" strong />
        <GBox x={540} y={22} w={190} h={215} label="Browse & Organize" />
        <GBox x={540} y={280} w={170} h={96} label="Utility" />

        {/* Draw connections first (behind nodes) */}
        {CONNECTIONS.main.map((c, i) => <Conn key={i} {...c} positions={P} />)}

        {/* Nodes */}
        <NCard id="home" {...P.home} isHub />
        <NCard id="notification" {...P.notification} isExt />
        <NCard id="recording" {...P.recording} isCore />
        <NCard id="detail" {...P.detail} isCore />
        <NCard id="search" {...P.search} />
        <NCard id="corememories" {...P.corememories} />
        <NCard id="settings" {...P.settings} />

        {/* Legend */}
        <g transform={`translate(30, ${H - 30})`}>
          <line x1={0} y1={0} x2={18} y2={0} stroke={C.textSoft} strokeWidth={1.5} />
          <text x={22} y={1} fontSize={8} fill={C.textMuted} fontFamily={F} dominantBaseline="central">Forward</text>
          <line x1={80} y1={0} x2={98} y2={0} stroke={C.accent} strokeWidth={1.5} />
          <text x={102} y={1} fontSize={8} fill={C.textMuted} fontFamily={F} dominantBaseline="central">Accent path</text>
          {showReturns && <g>
            <line x1={180} y1={0} x2={198} y2={0} stroke={C.textMuted} strokeWidth={1} strokeDasharray="5 3" opacity={0.4} />
            <text x={202} y={1} fontSize={8} fill={C.textMuted} fontFamily={F} dominantBaseline="central">Return</text>
          </g>}
        </g>
      </svg>
    );
  ...
  // ---- ONBOARDING MAP ----
  const OnboardingMap = () => {
    const mainIds = ["ob_signin", "ob_child", "ob_mic", "ob_notif", "ob_record", "ob_saved", "ob_paywall"];
    const cw = 115, ch = 44, gap = 36, px = 20, py = 44;
    const P = {};
    mainIds.forEach((id, i) => { P[id] = { x: px + i * (cw + gap), y: py, w: cw, h: ch }; });
    const recX = P.ob_record.x, savedX = P.ob_saved.x;
    P.ob_write = { x: (recX + savedX) / 2 - cw / 2 + 10, y: py + ch + 44, w: cw, h: ch };
    const W = px + mainIds.length * (cw + gap) + 80;
    const H = py + ch + 44 + ch + 40;

    return (
      <svg viewBox={`0 0 ${W} ${H}`} style={{ display: "block", width: "100%", maxWidth: W, height: "auto" }}>
        <Defs />
        {/* Main line connections */}
        {CONNECTIONS.onboarding.map((c, i) => <Conn key={i} {...c} positions={P} />)}

        {/* Nodes */}
        {mainIds.map(id => <NCard key={id} id={id} {...P[id]} />)}
        <NCard id="ob_write" {...P.ob_write} />

        {/* Exit to main app */}
        {(() => {
          const last = P.ob_paywall;
          const ex = last.x + last.w + 10;
          const ey = last.y + last.h / 2;
          return (
            <g>
              <line x1={ex} y1={ey} x2={ex + 24} y2={ey} stroke={C.textSoft} strokeWidth={1.5} markerEnd="url(#af)" />
              <rect x={ex + 30} y={ey - 14} width={78} height={28} rx={8} fill={C.tag} />
              <text x={ex + 69} y={ey + 1} textAnchor="middle" dominantBaseline="central" fontSize={10} fontWeight={600} fill={C.textSoft} fontFamily={F}>Main App</text>
            </g>
          );
        })()}
      </svg>
      ...
      <div style={{ width: 320, minWidth: 320, borderLeft: `1px solid ${C.border}`, background: C.card, padding: "20px 20px 24px", overflow: "auto", animation: "panelIn 0.25s ease-out", flexShrink: 0 }}>
        ...
        {conns.length > 0 && <div style={{ marginBottom: 18 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
            {conns.map((c, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <button onClick={() => onSelectScreen(c.to)} style={{ padding: "4px 10px", borderRadius: 10, background: c.type === "accent" ? C.accentSoft : C.tag, border: "none", fontSize: 11, fontWeight: 600, color: c.type === "accent" ? C.accent : C.textSoft, cursor: "pointer" }}>{SCREEN_INFO[c.to]?.label || c.to}</button>
                <span style={{ fontSize: 10, color: C.textMuted, fontFamily: F }}>{c.label}</span>
              </div>
            ))}
          </div>
        </div>}
        {inbound.length > 0 && <div style={{ marginBottom: 18 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: C.textMuted, letterSpacing: 0.8, textTransform: "uppercase", marginBottom: 8 }}>Reached From</div>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {inbound.map((c, i) => (
              <button key={i} onClick={() => onSelectScreen(c.from)} style={{ padding: "4px 10px", borderRadius: 10, background: C.tag, border: "none", fontSize: 11, fontWeight: 600, color: C.textSoft, cursor: "pointer" }}>{SCREEN_INFO[c.from]?.label || c.from}</button>
...
export default function App() {
  ...
  const mainScreens = [
    { id: "empty", label: "Welcome" }, { id: "home", label: "Home" },
    { id: "recording", label: "Recording" }, { id: "detail", label: "Entry Detail" },
    { id: "search", label: "Search" }, { id: "corememories", label: "♡ Core" },
    { id: "settings", label: "Settings" }, { id: "notification", label: "Notification" },
  ];
